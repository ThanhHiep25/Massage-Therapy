...
public class PaymentOrderServiceImpl implements PaymentOrderService {
    ...
    @Value("${vnp_OrderReturnUrl}")
    private String vnp_OrderReturnUrl;
    ...
    @Override
    public PaymentOrder createPayment(OrderResponse orderResponse, Long amount, String paymentMethod, String status) {
        // Lấy đối tượng Order từ database dựa trên ID trong OrderResponse
        Order order = orderRepository.findById(orderResponse.getId())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy đơn hàng với ID: " + orderResponse.getId()));

        PaymentOrder payment = new PaymentOrder();
        payment.setOrder(order); // Thiết lập đối tượng Appointment thực tế
    ...
    @Override
    public String createVNPayPayment(OrderResponse orderResponse, Long amount) {
        PaymentOrder payment = createPayment(orderResponse, amount, "VNPay", "PENDING");
        ...
        paymentOrderRepository.save(payment); // Lưu lại payment với transactionId mới
        ...
        params.put("vnp_ReturnUrl", vnp_OrderReturnUrl);
    ...
    public PaymentDTO handleVPNPaymentCallback(Map<String, String> params) throws UnsupportedEncodingException {
        ...
        Optional<PaymentOrder> optionalPayment = paymentOrderRepository.findByTransactionId(vnpTxnRef); // Tìm kiếm theo transactionId

        if (optionalPayment.isPresent()) {
            PaymentOrder paymentInfo = optionalPayment.get();
            ...
            paymentOrderRepository.save(paymentInfo);
...
//            String userEmail = "";
//            if (paymentInfo.getOrder().getUser() != null) {
//                userEmail = paymentInfo.getOrder().getUser().getEmail();
//            } else {
...
//        }
    ...
    public List<PaymentOrderResponse> findAll() {
        ...
        return payments.stream().map(PaymentOrderResponse::new).collect(Collectors.toList());
    }
